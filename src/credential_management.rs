...
use ctap_types::{
    authenticator::{
        ctap2::{
            self,
            credential_management::{CredentialProtectionPolicy, Response},
        },
        Error,
    },
    cose::PublicKey,
        ...
        + client::P256,
{
        ...
        if let Some(rp) = maybe_first_rp {
            ...
            if let Some(total_rps) = response.total_rps {
                if total_rps > 1 {
                    ...
                    self.state.runtime.cache_rp =
                        Some(CredentialManagementEnumerateRps(total_rps - 1, rp_id_hash));
                }
            }
        }
    ...
    pub fn next_relying_party(&mut self) -> Result<Response> {
        ...
        let (remaining, last_rp_id_hash) = match self.state.runtime.cache_rp {
            Some(CredentialManagementEnumerateRps(remaining, ref rp_id_hash)) => {
            ...
            match maybe_first_credential {
                ...
                Some(rk_entry) => {
                    ...
                    if remaining > 1 {
                        let rp_id_hash = response.rp_id_hash.as_ref().unwrap().clone();
                        self.state.runtime.cache_rp =
                            Some(CredentialManagementEnumerateRps(remaining - 1, rp_id_hash));
                    } else {
    ...
    pub fn next_credential(&mut self) -> Result<Response> {
        ...
        let (remaining, rp_dir, prev_filename) = match self.state.runtime.cache_rk {
            Some(CredentialManagementEnumerateCredentials(x, ref y, ref z)) => {
                (x, y.clone(), z.clone())
            }
            _ => return Err(Error::NotAllowed),
        };
    ...
    fn extract_response_from_credential_file(&mut self, rk_path: &Path) -> Result<Response> {
        ...
        let cose_public_key = match algorithm {
            ...
            SupportedAlgorithm::Totp => PublicKey::TotpKey(Default::default()),
        };
